...
player setVariable ["BIS_WL_funds", (player getVariable "BIS_WL_funds") - BIS_WL_transferCost, TRUE];
...
{_purchase_transfer_units lbAdd name _x} forEach BIS_WL_allWarlords;
...
[_purchase_transfer_units, _purchase_transfer_ok, _purchase_transfer_amount] spawn {
	...
	while {ctrlEnabled (_this # 1)} do {
		...
		if (typeName _value != typeName 0) then {_value = 0};
		...
		lbClear (_this # 0);
		{(_this # 0) lbAdd name _x} forEach ((BIS_WL_allWarlords select {side group _x == side group player}) - [player]);
		lbSort (_this # 0);
