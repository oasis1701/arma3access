...
for [{_i = 0}, {_i < _garrisonTotal}, {_i = _i + 1}] do {
	...
	_wp1 = _newGrp addWaypoint [[_spawnPos, 100, random 360] call BIS_fnc_relPos, 0];
	_wp2 = _newGrp addWaypoint [position leader _newGrp, 0];
	_wp3 = _newGrp addWaypoint [position leader _newGrp, 0];
	_wp3 setWaypointType "HOLD";
	{
		_x spawn {
			_start = position _this;
			_t = time + 60;
			waitUntil {sleep 1; time > _t || _this distance _start > 5};
			if (time > _t) then {
				["STUCK! %1 - deleting...", _this] call BIS_fnc_WLdebug; deleteVehicle _this
			};
		};
