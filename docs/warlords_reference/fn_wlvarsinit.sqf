	...
	BIS_WL_spawnProtectionCode = {
		...
		if ((side group _unit) in [WEST, EAST]) then {
			...
			if ([_unit, _base, TRUE] call BIS_fnc_WLInSectorArea && !(_base in [BIS_WL_currentSector_WEST, BIS_WL_currentSector_EAST])) then {
				...
				["Damage disabled for %1 (%2)", _unit, name _unit] call BIS_fnc_WLdebug;
				_unit setVariable ["BIS_WL_spawnProtectionEnd", (call BIS_fnc_WLSyncedTime) + 180, TRUE];
				_unit setVariable ["BIS_WL_damageAllowed", FALSE, TRUE];
				waitUntil {sleep 0.25; !alive _unit || !([_unit, _base, TRUE] call BIS_fnc_WLInSectorArea) || (_base in [BIS_WL_currentSector_WEST, BIS_WL_currentSector_EAST])};
				if (!(_unit getVariable ["BIS_WL_damageAllowed", TRUE])) then {
					[_unit, TRUE] remoteExec ["allowDamage", _unit];
					_unit setVariable ["BIS_WL_damageAllowed", TRUE, TRUE];
					["Damage enabled for %1 (%2)", _unit, name _unit] call BIS_fnc_WLdebug;
				};
			};
		};
	};
