...
if (BIS_WL_recentStart) then {
	...
	player setVariable ["BIS_WL_funds_backup", BIS_WL_startCP, TRUE];
} else {
	player setVariable ["BIS_WL_funds", if (BIS_WL_startCP > 50) then {50 min BIS_WL_maxCP} else {0}, TRUE];
	player setVariable ["BIS_WL_funds_backup", if (BIS_WL_startCP > 50) then {50 min BIS_WL_maxCP} else {0}, TRUE];
};
...
[] spawn {
	...
	while {TRUE} do {
		...
		while {!isNull (uiNamespace getVariable ["BIS_fnc_arsenal_cam", objNull])} do {
			...
			player removeMagazines "APERSMineDispenser_Mag";
			{
				if (getNumber (_cfgMags >> _x >> "weaponPoolAvailable") == 0) then {
					player removeMagazines _x;
				};
			} forEach magazines player;
			{
				if (getNumber (_cfgWpns >> _x >> "weaponPoolAvailable") == 0) then {
...
[] spawn {
	while {TRUE} do {
		waitUntil {player getVariable ["BIS_WL_damageAllowed", TRUE]};
		player allowDamage TRUE;
		waitUntil {sleep 0.25; !(player getVariable ["BIS_WL_damageAllowed", TRUE])};
	};
};
