...
Parameters: 	0 - Text (String)
		1 - Max number of lines to be shown at the same time (Number)
		2 - On-screen duration (Number)
		3 - Text color (Array [R,G,B,A])
		4 - Text outline (Bool)

Description: Eye-candy text effect.
*/
...
_textStructured = "<t size = '" + str ((1.45 call BIS_fnc_WLSubroutine_purchaseMenuGetUIScale)) + "' align = 'center' shadow = '" + (str _shadow) + "'>" + _textStructured + "</t>";
